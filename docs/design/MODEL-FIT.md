...
**The ruling (superseded 2026-07-13).** This document originally ruled here that
`--model` stays optional and silence merely stops being free. **The operator overruled
it: both `--model` and `--reason` are required; a spawn missing either fails.** That
implementation is underway (`spawn-mandate`). The original ruling and its reasoning are
kept below rather than deleted — per `docs/PHILOSOPHY.md` §10, a register that erases
what it superseded cannot show the same class of thing breaking twice — because the
reasoning is what the evidence actually supports and it is why the mandate is scoped the
way it is. **The *scoping* of the reason survives the overrule intact; only the
optionality does not.** Read the bullets below as the case for the mechanism the operator
chose, not as the current rule for whether the choice can be skipped — it cannot.

- ~~`swarm spawn` asks. The parent may still inherit — they may not do it *silently*. An
  explicit `inherit` is a legitimate, evidence-respecting answer; a *missing* one is a
  skipped step.~~ **Superseded:** the parent no longer merely gets asked — the tool
  refuses a spawn that does not answer. `inherit` (or equivalent) remains a legitimate
  value for `--model`; what changed is that *some* value, and a `--reason`, are no longer
  optional.
- **The reason must be scoped to the ladder's question — and this is the part that
  survives the overrule completely.** The compelled field is only worth making mandatory
  *because* it asks *"can I cheaply tell that this child was wrong?"* — **not** "why is
  this model good." This distinction is load-bearing, and it comes from the one objection
  the review could not defeat:
  ...
  That is the sharpest thing said in this workstream, and it does not go away just
  because the field is now required — if anything, a *required* field that asked "why is
  this model good" would manufacture fabrication at 100% of spawns instead of some of
  them. A parent asked *"why is Haiku right for this?"* must assert knowledge about a
  model's competence **that nobody in this repo has** — and will assert it fluently,
  which is precisely the confident-wrong artifact §6 exists to prevent. A parent asked
  *"can you cheaply tell if this child is wrong?"* is asked about **their own
  verification capacity, and they always know the answer**: *"I will read every line of
  this doc anyway"* is knowable at spawn without a single benchmark. **The question the
  ladder asks is answerable honestly today. The question "which model is smarter" is
  not.** The mandate must scope its required `--reason` to the former, or it manufactures
  the very fabrication it was built to prevent — the operator's overrule raises the
  stakes on getting this scoping right, it does not remove the need for it.
- **Give the field a reader, in the same change, or do not ship it.** The record
  ...
  the tree. **A mandate without a reader is write-only, and should be opposed.** This,
  too, survives the overrule unchanged — a *required* field with no reader is write-only
  more expensively than an optional one was.

**The evidence, and why it now supports the mandate more strongly than it supported the
optional ruling.** 17 documented cases where the compelled field *changed what the agent
did* (§ above, `docs/audit/org-review-falsifier-2026-07-12.md:84-87`), against 0 where it
fired and was ignored; 0 of 135 recorded falsifier statements were unfalsifiable mush.
That evidence was the basis for the original, optional ruling. It applies with at least
equal force to the required version: nothing in the 17-case record suggests compelled
articulation stops working once it is no longer skippable — the record shows it working
*despite* being skippable in every other doctrine-compelled field this repo runs. Making
it required removes the one gap the optional ruling left open (142 of 143 spawns took the
silent default), which is the gap the operator's overrule is aimed at closing.

**What this repo ships now, and what it does not.** The doctrine and the help text (§5b)
were, at the time this document was first written, proposed as the whole of this PR's
change, with the CLI mandate specified and recommended but deliberately left
unimplemented pending its own reviewed change (blast radius measured by patching a
scratch copy of `bin/swarm` and running the suite: **at least** ~9 test call-sites and 12
doc files — *a floor, not a total, per the omission caveat in §4*). **That sequencing
argument is also superseded** — the operator decided not to wait and is having the
mandate implemented directly (`spawn-mandate`). The doctrine and help-text changes in
this PR still stand on their own merits and are not redundant with the mandate: they are
the frame the mandate's error text points back to.
...
nobody in the tree selected. (§5 now records that this mechanism is no longer optional
future work: the operator ordered it built, and `--model inherit` said out loud is exactly
what a required-but-not-silent `--model` flag gives you.)
