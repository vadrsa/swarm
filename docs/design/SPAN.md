...
0. **A warm name** — before spawning fresh: is this shape of work already
   owned by a warm name? Reuse is cheaper than any rung above it. A recurring
   shape of work IS a nameable next task — this is where the close rule and
   rung 0 meet: close eagerly, but do not destroy the warm name reuse depends
   on when the shape is known to recur.
